...
\fi
\ifshowtranssecond %(校正)
\chapter{デザイン例 (L10642 仮訳 12/22 01/02 校正1 aki)} %(仮訳 12/22 aki  11章 ========================================================================)
\fi
...
\ifshowtranssecond %(校正 6/20 mune10 校正1 01/02 aki)
このセクションでは、FIFOバッファなど、より大きなデザインのビルディングブロックとして使用するいくつかの小さなデジタルデザインを探索します。
別の例として、シリアル・インターフェース(UARTとも呼ばれます)を設計しますが、これはFIFOバッファを使用します。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{FIFO バッファ (L10098 仮訳 12/22 校正1 01/02 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/22 校正1 01/02 aki)
ライタとリーダの間にバッファを設けることで、書き込み(送信者)とリーダ(受信者)を切り離すことができます。
共通のバッファは、先入れ先出し(\href{https://en.wikipedia.org/wiki/FIFO_(computing_and_electronics)}{FIFO})バッファです。
...
\ifshowtranssecond %(仮訳 12/22 校正1 01/02 aki)
FIFOは初期状態では空で、\code{エンプティ}信号で状態が示されています。
空のFIFOからの読み出しは通常未定義です。
データが書き込まれても読み出されない場合、FIFOは\code{フル}になります。
...
\code{エンプティ}信号は、リーダ側のフロー制御を担当します。
\fi
...
% 超意訳
% \code{深さ}\code{FifoRegister}から、\code{深さ}ステージを持ったバブルFIFOを構築することができます。
% Scala\code{Array}につめこんでステージを作成します。
Scalaの\code{Array}にfillを使って、\code{深さ}\code{FifoRegister}個分のBubbleをつめこんで、そのバッファステージの深さを持ったバブルFIFOを構築します。
...
\fi
\ifshowtranssecond %(校正)
\section{シリアルポート (L10368 仮訳 12/23 校正1 01/02 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/23 校正1 01/02 aki)
...
ステートが\code{フル}になり、下流側の受信機が\code{ready}になったら、下流側のデータ転送が行われ、ステートを\code{空}に戻します。
\fi
...
\ifshowtranssecond %(仮訳 12/23 校正1 01/02 aki)
\longlist{code/uart_rx.txt}{A receiver for a serial port.}{lst:uart:rx}
コード~\ref{lst:uart:rx}にレシーバー(Rx)のコードを示します。
...
8ビットシフトインした後、\code{valReg}にバイトデータが有効であることを通知します。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{ FIFO設計のバリエーション (L11322 仮訳 12/23 校正1 01/03 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/23 校正1 01/03 aki))
...
\fi
\ifshowtranssecond %(校正)
\subsection{FIFOのパラメータ化 (L1135 仮訳 12/24 校正1 01/03 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/24 校正1 01/03 aki)
...
\fi
\ifshowtranssecond %(校正)
\subsection{バブルFIFOの再設計 (L11471 仮訳 12/24 校正1 01/03 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{ダブルバッファFIFO (L11554 仮訳 12/24 校正1 01/03 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{レジスタメモリ付きFIFO (L11646 仮訳 12/24 校正1 01/03 aki)}
\fi
...
固定サイズのFIFOキューは通常\myref{https://en.wikipedia.org/wiki/Circular_buffer}{循環バッファ}として実装されます。
...
\fi
\ifshowtranssecond %(校正)
\subsection{オンチップメモリ付きFIFO (L11872 仮訳 12/27 校正1 01/03 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 校正1 01/03 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{演習 (L11980 仮訳 12/27 校正1 01/03 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 校正1 01/03 aki)
この演習節には、2つの演習が含まれているので、少し長くなります : 
...
\fi
\ifshowtranssecond %(校正)
\subsection{バブルFIFOを探る (12004 仮訳 12/27 校正1 01/03 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 校正1 01/03 aki)
また、FIFOソースには、さまざまな読み書き動作を行って、\myref{https://en.wikipedia.org/wiki/Value_change_dump}{value change dump (VCD)}形式の波形を生成するテスタも含まれています。
...
\ifshowtranssecond %(仮訳 12/27 校正1 01/03 aki)
これらの欠点は、\myref{https://en.wikipedia.org/wiki/Circular_buffer}{循環バッファ}を用いたFIFO実装によって解決することができます。
...
\fi
\ifshowtranssecond %(校正)
\subsection{UART (L12162 仮訳 12/27 校正1 01/03 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{FIFO探査 (L12296 仮訳 12/27 校正1 01/03 aki)}
\fi
