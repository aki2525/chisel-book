...
\fi
\ifshowtranssecond %(校正)
\chapter{有限オートマトン(FSM)(L8048 仮訳 12/12 校正1 01/01 aki)}%(校正  第8章  ========================================================================)
\fi
...
\ifshowtranssecond %(仮訳 12/12 校正1 01/01 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{有限オートマトンの基本 (L8124 仮訳 12/12 校正1 01/01 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/12 校正1 01/01 aki)
\myref{https://en.wikipedia.org/wiki/State_diagram}{状態図}は、そのようなFSMの動作を視覚的にあらわして記述したものです。
...
\ifshowtranssecond %((仮訳 12/15 校正1 01/01 aki)
...
\emph{悪いできごと}が発生したときに\emph{クリア}入力が無視されることを示して、表を単純化しています。
...
\ifshowtranssecond %(校正 6/20 mune10 校正1 01/01 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{ミーリー FSMで出力を高速化 (L78454 仮訳 12/15 校正1 01/01 aki)}
\fi
...
以前にもこのChiselで記述されたすごい1行を見たことがあります:
\fi
...
\ifshowtranssecond %(仮訳 12/15 校正1 01/01 aki)
図~\ref{fig:fsm-rising}に立ち上がりエッジ検出器の回路図を示します。
...
\ifshowtranssecond %(仮訳 12/15 校正1 01/01 aki)
出力が入力にも依存している場合、すなわち、FSMの入力と出力の間に組合せ経路がある場合、これを\myref{https://en.wikipedia.org/wiki/Mealy_machine}{ミーリーマシン}と呼びます。
\fi
...
\fi
\ifshowtranssecond %(校正))
\section{ムーア対ミーリー (L78692 仮訳 12/19 校正1 01/01 aki))}
\fi
...
\ifshowtranssecond %(仮訳 12/19 校正1 01/01 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{演習 (L8843 仮訳 12/19 校正1 01/01 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/19 校正1 01/01 aki)
FSMの典型的な例は、信号機コントローラです(\cite[Section~14.3]{dally:vhdl:2016}を参照ください)。
信号制御器は、赤から緑への切り替えの際に、交差点内の両方の道路が進入禁止信号(赤とオレンジ)を待っている間の段階がなくてはなりません。
