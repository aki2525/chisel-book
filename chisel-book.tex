...
\fi
\ifshowtranssecond %(校正)
\chapter{ 順序回路ブロック (L5709 TODO)} %(仮訳 12/06 aki) 第6章  ========================================================================)
\fi
...
\ifshowtranssecond %(仮訳 12/06  校正1 12/29 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{レジスタ (L5743 仮訳 12/06 校正1 12/29 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/06 校正1 12/29 aki)
順序回路を構築するための基本的な要素は、レジスタです。
レジスタは\myref{https://en.wikipedia.org/wiki/Flip-flop_(electronics)\#D_flip-flop}{D flip-flops}の集まりです。
D フリップフロップは、クロックの立ち上がりエッジで入力の値をキャプチャして保持し、出力します。
...
これには、入力と出力の\code{D} \code{Q}があります。
各レジスタにはまた\code{クロック}信号の入力があります。
このグローバルクロック信号は通常、同期回路内のすべてのレジスタに接続されているので、通常は回路図に描かれていません。
ボックスの下部にある小さな三角形は、クロック入力を表し、これはレジスタであることを示しています。
...
\ifshowtranssecond %(校正 6/20 mune10 校正1 12/29 aki)
...
\ifshowtranssecond %(仮訳 12/06 校正1 12/29 aki)
まず、我々は、レジスタを定義し、名前を付けます。
第二に、我々は、レジスタの入力に\code{delayIn}信号を接続します。
レジスタの名前は、文字列\code{Reg}が含まれていることにも注意してください。
簡単に組み合わせ回路と順序回路を区別するために、名前の一部にマーカーとして\code{Reg}をつけておくのが一般的です。
また、Scala(ですからChiselも)の命名規則は通常\myref{https://en.wikipedia.org/wiki/Camel_case}{CamelCase}が使われます。
変数名は小文字で始まり、クラスは大文字で始まります。
\fi
...
そのふるまいは、時間の経過とともに変化する信号を示すダイアグラムによってみることができます。このようなダイアグラムは、ウェーブフォームまたは\myref{https://en.wikipedia.org/wiki/Digital_timing_diagram}{timing diagram}と呼ばれています。
\fi
...
\ifshowtranssecond %(仮訳 12/06 校正1 12/29 aki)
図~\ref{fig:register-wave}に、リセットと一部の入力データが適用されたレジスタのウェーブフォームを示します。
時間は左から右へと進みます。
...
次のクロックサイクルでは\code{リセット}は\code{0}になり、\code{inVal}の値は次の立ち上がりエッジ(ラベルC)でキャプチャされます。
それ以降は\code{リセット}は\code{0}のままで、レジスタ出力は入力信号から1クロックサイクル遅れで追従します。
\fi
...
\ifshowtransfirst %(自動翻訳)
図〜\ref{fig:register-enable}は、イネーブル付きレジスタの回路図を示しています。
...
ほとんどの場合、イネーブルをハイ(\code{true})にすると、レジスタは1クロックサイクルの遅延で入力に従ってやつで、います。
4クロックサイクル目にのみ\code{イネーブル)がLowになり、Dの立ち上がりエッジでレジスタは値(5)を保持します。
\fi
...
\subsection{Counting Up and Down}
\fi
\ifshowtranssecond %(校正 校正1 12/29 aki)
\subsection{カウントアップとダウン (L6268 仮訳 12/06 校正1 12/29 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正 6/20 mune10 校正1 12/29 aki)
\noindent 私たちはコーディングをしていて、より多くのカウンタを使っているので、カウンタを生成するためのパラメータを持つ関数を定義することができます。
\fi
...
\ifshowtranssecond %(校正 6/20 mune10 校正1 12/29 aki)
注、すべてのカウンタ例で、値はNを含めて\code{0}から\code{N}の間にあることに注意してください。
10クロックサイクルをカウントしたい場合は、\code{N}を9に設定する必要があります。
このように、カウントと値が1つずれるのは、\myref{https://en.wikipedia.org/wiki/Off-by-one_error}{植木算}の古典的な例です。
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{カウンタによるタイミングの生成 (L6371 12/06 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/07 校正1 12/29 aki)
...
\ifshowtranssecond %(仮訳 12/07 校正1 12/29 aki)
\noindent 図~\ref{fig:tick-count-wave}は、ティックのウェーブフォームとティック$n$(クロックサイクル)ごとにインクリメントするスローカウンタのウェーブフォームを示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/07 校正1 12/29 aki)
幅推論ではレジスタのサイズを決める必要がありますが、レジスタ定義時の型や初期化値で明示的に幅を指定した方が良いでしょう。明示的に幅を定義することで、リセット値\code{0.U}の結果として1ビットの幅を持つカウンタが生成されてしまい、驚くことがなくなります。
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{「オタク」カウンター (L6538 仮訳 12/07 校正1 12/29 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/07 校正1 12/29 aki)
多くの人が、たまに\myref{https://en.wikipedia.org/wiki/Nerd}{オタク}になった気分になることがあります。
例えば、カウンタ/ティック生成の高度に最適化されたバージョンを設計したいとします。
...
\fi
\ifshowtranssecond %(校正)
\subsection{タイマー (L6610 仮訳 12/08 校正1 12/29 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 校正1 12/29 aki)
...
\ifshowtranssecond %(仮訳 12/08 校正1 12/29 aki)
リスト~\ref{lst:timer}はタイマーのChiselコードを示しています。
...
\fi
\ifshowtranssecond %(校正)
\subsection{パルス幅変調(PWM) (L6721 仮訳 12/08 校正1 12/30 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 校正1 12/30 aki)
...
信号が高い時間の割合は、また、デューティ・サイクルと呼ばれます。
...
\ifshowtranssecond %(仮訳 12/08 校正1 12/30 aki)
PWM信号に\myref{https://en.wikipedia.org/wiki/Low-pass_filter}{low-pass filter} を加えると、単純な\myref{https://en.wikipedia.org/wiki/Digital-to-analog_converter}{digital-to-analog converter}になります。
...
\ifshowtranssecond %(校正 6/20 mune10 校正1 12/30 aki)
次のコード例では、10クロックサイクルごとに 3クロックサイクルのハイ(1)の波形を生成します。
\fi
...
実数の除算はハードウェア上では非常に高価なので、単に右に10ビットシフトするだけで、$2^{10} = 1024$による除算と同じ結果を得てその値を使います。
...
\fi
\ifshowtranssecond %(校正)
\section{シフトレジスタ (L6919 仮訳 12/08 校正1 12/30 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 校正1 12/30 aki)
\myref{https://en.wikipedia.org/wiki/Shift_register}{shift register}は、順番に接続されたフリップフロップの集合体です。
...
\fi
\ifshowtranssecond %(校正)
\subsection{パラレル出力付きシフトレジスタ (L6992 仮訳 12/08 校正1 12/30 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{パラレルロード付きシフトレジスタ (L7067 仮訳 12/08 校正1 12/30 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/09 校正1 12/30 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{メモリー (L7128 仮訳 12/09 校正1 12/30 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/09 校正1 12/30 aki)
ChiselにあるVecのRegで表されるメモリはレジスタの集まりで構築することができます。
しかし、これはハードウェアでは高価であるので、大きなメモリ構造は\myref{https://en.wikipedia.org/wiki/Static_random-access_memory}{SRAM}で構築します。
...
\fi
\ifshowtranssecond %(校正 6/20 mune10 校正1 12/30)
...
リスト~\ref{lst:memory}は、バイト単位の入出力データとライトイネーブルを持つ1~KiBのメモリを実装したコンポーネント\code{Memory}を示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/09 校正1 12/30 aki)
リスト\ref{lst:memory:forward}に、フォワード(転送)回路を含む同期メモリのChiselコードを示します。
...
\fi
\ifshowtranssecond %(校正)
\section{演習 (L6986 7332 仮訳 12/09 校正1 12/30 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\chapter{入力処理 (L7452 仮訳 12/10 aki)}%(校正  第7章  ========================================================================)
\fi
...
\ifshowtranssecond %(仮訳 12/10 aki)
後者の2つの問題、デバウンススイッチとフィルタリングノイズは、外付けのアナログ部品でも解決できます。
しかし、これらの問題はデジタル側で対処するのが(コスト的に)効率的です。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{非同期入力 (L7502 仮訳 12/10 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/10 aki)
システムクロックに同期しない入力信号は非同期信号と呼ばれます。
これらの信号は、フリップフロップに入力されるセットアップおよびホールド時間に違反するでしょう。
この違反により、フリップフロップの\myref{https://en.wikipedia.org/wiki/Metastability_(electronics)}{メタスタビリティ}が発生する可能性があります。
メタスタビリティの結果、出力に0と1を繰り返したり 結果が発振のようになるでしょう。
しかし、しばらくするとフリップフロップは0または1で安定します。
\fi
...
\ifshowtranssecond %(仮訳 12/10 aki)
図~\ref{fig:synchronizer}は、同期回路と外の世界の境界を示しています。
入力同期器は2つのフリップフロップで構成されています。
Chiselの入力同期器は気がきいていて、2つのレジスタをインスタンス化してくれます。
\fi
...
\ifshowtranssecond %(仮訳 12/10 aki)
すべての非同期外部信号には入力同期器が必要です。
\footnote{例外は、入力信号が同期出力信号に依存している場合で、最大伝搬遅延が分かっている場合です。典型的な例は、非同期SRAMをマイクロプロセッサなどの同期回路に接続する場合です。}
また、外部のリセット信号と同期させる必要があります。
リセット信号は、回路内の他のフリップフロップのリセット信号として使用される前に、2つのフリップフロップを通過しなければなりません。
具体的には、リセットのディアサートはクロックと同期している必要があります。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{デバウンス (L7640 仮訳 12/10 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/10 aki)
スイッチやボタンは、オンとオフの間で切り替わるのに時間がかかる場合があります。
切り替わりの間、スイッチはこれらの2つの状態の間であばれます。
このような信号をさらに処理せずに使用すると、必要以上に多くの切り替わりイベントを検出してしまう可能性があります。
1つの解決策は、時間を使用してこのバウンドをフィルタリングすることです。
バウンスの最大時間を$t_{bounce}$と仮定して、入力信号を$T > t_{bounce}$の周期でサンプリングします。サンプリングされた信号は、さらに下流でのみ使用します。
\fi
...
\fi
\ifshowtranssecond %(校正)
\chapter{有限オートマトン（FSM）(L8048 仮訳 12/12 aki)}%(校正  第8章  ========================================================================)
\fi
...
\ifshowtranssecond %(仮訳 12/12 aki)
有限状態機械(FSM)は、デジタル設計の基本的な構成要素です。
FSMは、状態と状態の間の遷移とその(ガードされた)条件を記述しています。
FSMは、リセット時に設定される初期状態があります。
FSM は同期シーケンシャル回路と呼ばれることもあります。
\fi
...
しかし、これは実用的ではないかもしれません、例えば、ラップトップ コンピュータを単一のFSMとして記述してみてください。
...
\fi
\ifshowtranssecond %(校正)
\section{有限オートマトンの基本 (L8124 仮訳 12/12 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/12 aki)
図~\ref{fig:fsm}は、FSMの回路図を示しています。
レジスタには現在の\code{状態}が格納されています。
次の状態ロジックは、現在の\code{状態}と入力(\code{in})から次の状態値(\code{next\_state})を計算します。
次のクロックティックで\code{状態}が\code{next\_state}になります。
出力ロジックは、出力(\code{out})を計算します。
出力は現在の状態にのみ依存するので、この状態マシンを\myref{https://en.wikipedia.org/wiki/Moore_machine}{ムーアマシン}と呼びます。
\fi
...
\ifshowtranssecond %(仮訳 12/12 aki)
\myref{https://en.wikipedia.org/wiki/State_diagram}{状態図}は、そのような FSM の動作を視覚的に表して記述したものです。
...
FSMは3つの状態 :アラームレベルを示す、\emph{緑}、\emph{橙}、\emph{赤}があります。
...
\ifshowtranssecond %((仮訳 12/15 aki)
...
\emph{悪いできごと}が発生したときに\emph{クリア}入力が気にしないことを示すことにより、表を単純化しています。
...
リスト~\ref{lst:fsm:alarm}はアラームFSMのChiselコードを示しています。
...
しかし、それらの低レベルの判断は合成ツールに任せて、読みやすいコードを目指します。
...
状態のリストには、\code{Enum}インスタンスが\emph{使われ}ます。
...
ステートレジスタのChiselスイッチで、すべてのステートをカバーします。
それぞれのブランチ\code{で}、入力に依存する次の状態ロジックを記述し、状態レジスタに新しい値を割り当てます。
\fi
...
\ifshowtranssecond %(仮訳 12/15 aki)
\noindent 最後に、重要なことですが、状態が\emph{赤}のときに\emph{鳴るベル}の出力がtrueになるように記述します。
\fi
...
\ifshowtranssecond %(仮訳 12/15 aki)
VerilogやVHDLでは普通に行われているように、レジスタ入力には\code{next\_state}信号を導入して\emph{いない}ことに注意してください。
VerilogやVHDLのレジスタは特殊な構文で記述されており、組み合わせブロック内では割り当て(および再割り当て)ができません。
そのため、組み合わせブロック内で計算された追加信号が導入され、レジスタ入力に接続されます。
...
\fi
\ifshowtranssecond %(校正)
\section{ミーリー FSMで出力を高速化 (L78454 仮訳 12/15 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/15 aki)
図~\ref{fig:fsm-rising}に立ち上がりエッジ検出器の回路図を示す。
現在の入力が1で、最後のクロックサイクルの入力が0だった場合、出力は1クロックサイクルの間1になります。
状態レジスタは、次の状態が入力されただけの単一のＤフリップフロップです。
これを1クロックサイクルの遅延要素と考えることもできます。
...
\ifshowtranssecond %(仮訳 12/15 aki)
出力が入力にも依存している場合、すなわち、FSMの入力と出力の間に組合せ経路がある場合、これを\myref{https://en.wikipedia.org/wiki/Mealy_machine}{みーリーマシン}と呼びます。
\fi
...
ムーア型FSMと同様に、レジスタには現在の\code{状態}が格納され、次の状態ロジックは現在の\code{状態}と入力(\code{in})から次の状態値(\code{next\_state})を計算します。
次のクロックティックで\code{状態}は\code{next\_state}になります。
...
また、自己遷移を描画することにも注意してください。例えば、入力が\code{0}の場合、状態\code{zero}ではFSMは状態\code{zero}のままで、出力は0になります。
...
\ifshowtranssecond %(仮訳 12/15 aki)
リスト~\ref{lst:fsm:rising}は、ミーリーマシンを用いた立ち上がりエッジ検出のためのChiselコードです。
...
\section{Moore versus Mealy}
\fi
\ifshowtranssecond %(仮訳 12/19 aki))
\section{ムーア対ミーリー (L78692 仮訳 12/19 aki))}
\fi
...
状態\code{パルス}は、シングルサイクルパルスを生成するために必要です。
FSMは、1クロックサイクルの間\code{パルス}状態を維持し、入力が再び0になるのを待って、開始状態の\code{zero}に戻るか、\code{one}の状態に戻ります。
...
\ifshowtranssecond %(仮訳 12/19 aki)
リスト~\ref{lst:fsm:rising:moore}は立ち上がりエッジ検出回路のムーア版を示しています。
それは、ミーリーまたは直接実装したものに比べて2倍のDフリップフロップを使用しています。
したがって、結果として得られる次の状態ロジックも、ミーリーまたは直接実装されたバージョンよりも大きくなります。
\fi
...
\ifshowtranssecond %(仮訳 12/19 aki)
図~\ref{fig:rising}は立ち上がりエッジ検出FSMのMealy版とMoore版の波形を示しています。
ミーリー出力は入力された立ち上がりエッジに忠実に追従しているのに対し、ムーア出力はクロックティック後に立ち上がります。
また、ムーア版の出力は1クロック周期の幅があるのに対し、Mealy版の出力は通常1クロック周期以下であることがわかります。
\fi
...
ムーアFSMの状態レジスタとの組み合わせパスが切断されているため、ムーアFSMの通信ではには上記の問題は存在しません。
\fi
...
実質的にミーリマシンである立ち上がりエッジ検出などの小さな回路も問題ありません。
\fi
...
% This is a boring example, maybe I can find something more interesting
In this chapter, you have seen many examples of very small FSMs.
では、そろそろ\emph{実際}に FSM のコードを書いてみましょう。
...
\ifshowtranssecond %(仮訳 12/19 aki)
FSMの典型的な例は、信号機コントローラです(see~\cite[Section~14.3]{dally:vhdl:2016})。
信号制御器は、赤から緑への切り替えの際に、交差点内の両方の道路が進入禁止信号（赤とオレンジ）を持っている間の段階がなくてはなりません。
...
\chapter{Communicating State Machines}
\fi
\ifshowtranssecond %(仮訳 12/20 aki)
\chapter{コミュニケートステートマシン (L8903 TODO)} %(仮訳 12/20 aki  第9章  ========================================================================)
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/20 aki)
\section{ライトフラッシャーの例 (L8944 仮訳 12/20 aki}
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
直接実装のFSM\footnote{状態図を~\cite[p.~376]{dally:vhdl:2016}に示します。}は27の状態を持つ。
入力待ちの初期状態が1つ、3つのオン状態のための$3 \times 6$状態、オフ状態のための$2 \times 4$状態があります。
...
\ifshowtranssecond %(仮訳 12/20 aki)
リスト~\ref{lst:flasher:master}は、マスターFSMを示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
このマスターFSMとタイマーを用いた解決法では、マスターFSMコードに冗長性が残っています。
ステートの\code{flash1}、\code{flash2}、および\code{flash3}は同じ機能を実行しており、ステートの\code{space1}と\code{space2}も同様です。
残りのフラッシュの数を第２のカウンタにくくりだすことができる。
そうすると、マスターFSMは次の3つの状態に減る:
\code{off}, \code{flash}, と\code{space}です。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
図~\ref{fig:flasher2}は、マスターFSMとカウントする2つのFSMのデザインを示しています。
1つ目のFSMは、\emph{オン}と\emph{オフ}の間隔の長さのクロック サイクルをカウントします。
...
\ifshowtranssecond %(仮訳 12/20 aki)
\noindent カウンターは\emph{残り}のフラッシュをカウントし、タイマーが終了したステートの\code{合間}でデクリメントされるので、3回のフラッシュに対して2回のロードとなることに注意してください。
リスト~\ref{lst:flasher2:master}はダブルリファクタリングされたフラッシャーのマスターFSMを示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
マスターFSMが3つの状態に削減されただけでなく、現在のソリューションはより良いコンフィグレーションが可能です。
...
\fi
\ifshowtranssecond %(仮訳 12/20 aki)
\section{データパスを持つステートマシン (L9194 仮訳 12/20 aki)}
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/20 aki)
\subsection{ポップカウントの例 (L9242 仮訳 12/20 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
図~\ref{fig:popcnt-fsmd}に示すFSMDは、また\myref{https://en.wikipedia.org/wiki/Hamming_weight}{Hamming weight}と呼ばれるポップカウントを計算する例です。
ハミングウェイトは、ゼロシンボルとは異なるシンボルの数です。
バイナリ文字列の場合、これは数`1'となります。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
popcountユニットはデータ入力\code{din}と結果出力\code{popCount}を含み、両方ともデータパスに接続されています。
...
FSMは入力を待つ\code{アイドル}状態からスタートします。
データが到着し、有効な信号が送られてくると、FSMはシフトレジスタをロードするために、状態\code{ロード}に進みます。
FSMは次のステート\code{カウント}に進み、そこで`1'の数が順次カウントされます。
シフトレジスタ、加算器、アキュムレータレジスタ、ダウンカウンタを使用して計算を行います。
ダウンカウンタがゼロになると計算が終了し、FSMは\code{終了}の状態に移行する。
ここでFSMはpopcount 値が消費される準備ができたことを有効な信号で通知します。
受信機からの準備完了の信号でFSMは\code{アイドル}状態に戻り、次のpopcountを計算する準備ができています。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
リスト~\ref{lst:pop:top}で示されるトップレベルのコンポーネントはFSMとデータパスコンポーネントをインスタンス化し、バルク接続で接続します。
\fi
...
`1'の数をカウントするために、\code{shf}レジスタを右にシフトし、クロックサイクルごとに最下位ビットを\code{cnt}に加算する。
...
結果が読み出された場合は\code{popCntValid}をアサートすることで、FSWは\code{アイドル}状態に戻ります。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
\code{ロード}信号では、\code{regData}レジスタが入力でロードされ、\code{regPopCount}レジスタが0にリセットされ、カウンタレジスタ\code{regCount}が実行するシフト数に設定されます。
\fi
...
\ifshowtranssecond %(仮訳 12/20 aki)
そうでない場合は、\code{regData}レジスタが右にシフトされ、\code{regData}レジスタの最下位ビットが\code{regPopCount}レジスタに追加され、カウンタが0になるまでデクリメントされます。
カウンタが０の時、出力はポップカウントを含みます。
リスト~\ref{lst:pop:data}はpopcount回路のデータパスのためのChiselコードを示しています。
\fi
...
リスト~\ref{lst:pop:fsm}はFSMのコードを示しています。
\fi
...
\section{Ready-Valid Interface}
\fi
\ifshowtranssecond %(仮訳 12/21 aki)
\section{Ready-Valid インターフェース (L9462 仮訳 12/21 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
ready-validインタフェース~\cite[p.~480]{dally:vhdl:2016}は、送信側(プロデューサ)では\code{データ}と\code{バリッド}信号、受信側(コンシューマ)では\code{レディ}信号で構成される単純なフロー制御インタフェースです。
図~\ref{fig:readyvalid}は、ready-valid接続を示しています。
送信側は\code{データ}があるときに\code{バリッド}をアサートし、受信側は1ワードのデータを受信する準備ができているときに\code{レディ}をアサートする。
データの送信は、\code{バリッド}と\code{レディ}の両方の信号がアサートされたときに行われます。
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid1}は、受信機が送信機がデータを持つ前に(クロックサイクル１から)受信機が\code{レディ}信号を出すready-validトランザクションのタイミング図を示しています。
データ転送はクロックサイクル3で行われます。
クロックサイクル4からは、送信者がデータを持っていることも受信者が次の転送の準備ができていることもありません。
受信機がすべてのクロックサイクルでデータを受信できる場合、それは``常時レディ''インターフェースと呼ばれ、\code{レディ}は\code{true}にハードコードすることができます。
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid2}は、送信者が受信機の準備が整う前に(クロックサイクル１から)\code{バリッド}な信号を送信するready-validトランザクションのタイミング図を示しています。
データ転送は、クロックサイクル3で行われる。
クロックサイクル4以降は、送信者も受信者も次の転送の準備ができていません。
``常時レディ''インターフェイスと同様に、常に有効なインターフェイスを想定することができます。
しかし、その場合はおそらく\code{レディ}信号の変化でデータが変わることはなく、単純にハンドシェイク信号を落とすことになるでしょう。
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid2}は、ready-validインタフェースの更なるバリエーションを示している。
クロックサイクル1では、両方の信号(\code{レディ}および\code{バリッド})が1クロックサイクルの間だけアサートされ、\code{D1}のデータ転送が起こる。
...
\ifshowtranssecond %((仮訳 12/21 aki)
このインターフェイスを構成可能にするために、\code{レディ}とnot \code{バリッド}は他の信号との組み合わせに依存することは許されていません。
...
\ifshowtranssecond %(仮訳 12/21 aki)
\noindent \code{DecoupledIO}バンドルは、\code{データ}の型でパラメータ化されています。
Chiselで定義されたインターフェースは、データのフィールド\code{ビット}を使用します。
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
1つの疑問が残るのは、アクティブな状態でデータ転送が行われて\emph{いない}場合に、\code{レディ}または\code{バリッド}な状態がデ・アサートされる可能性があるかどうかです。
例えば、受信機はしばらくの間準備ができていてデータを受信していないかもしれないが、他のイベントのために準備ができていない状態になるかもしれない。
送信側でも同じことが考えられ、データが有効なのはいくつかのクロックサイクルだけで、データ転送が起こらず有効でなくなることがあります。
...
\ifshowtranssecond %(仮訳 12/21 aki)
Chiselでは、\code{DecoupledIO}クラスを使用した場合、\code{レディ}と\code{バリッド}のシグナリングには何の制約もありません。
...
\ifshowtranssecond %(仮訳 12/21 aki)
\begin{quote}
\code{ReadyValidIO}のサブクラスで、\code{バリッド}がHighで\code{レディ}がLowのサイクルの後は\code{ビット}の値を変更しないことを約束します。
さらに、一度\code{バリッド}が上がった後は、\code{レディ}が上がった後まで値が下がることはありません。
\end{quote}
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
\noindent これは、クラスを使用して強制することができない規約であることに注意してください。
\fi
...
AXIは、一度\code{レディ}または\code{バリッド}がアサートされると、データ転送が起こるまでアサートが解除されないようにインターフェイスを制限します。
\fi
...
\chapter{Example Designs}
\fi
\ifshowtranssecond %(仮訳 12/22 aki)
\chapter{デザイン例 (L10642 仮訳 12/22 aki)} %(仮訳 12/22 aki  11章 ========================================================================)
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/22 aki)
\section{FIFO バッファ (L10098 仮訳 12/22 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/22 aki)
ライタとリーダの間にバッファを設けることで、書き込み(送信者)とリーダ(受信者)を切り離すことができます。
共通のバッファは、先入れ先出し(\href{https://en.wikipedia.org/wiki/FIFO_%28computing_and_electronics%29}{FIFO})バッファである。
...
\ifshowtranssecond %(仮訳 12/22 aki)
FIFOは初期状態では空で、\code{エンプティ}信号で区切られています。
空のFIFOからの読み出しは通常未定義です。
データが書き込まれても読み出されない場合、FIFO は\code{フル}になります。
フルFIFOへの書き込みは通常無視され、データは失われます。
つまり、\code{エンプティ}と\code{フル}信号はハンドシェイク信号として機能します。
\fi
...
\ifshowtranssecond %(仮訳 12/22 aki)
FIFOのいくつかの異なる実装が可能です :
たとえば、オンチップメモリと読み出しポインターおよび書き込みポインターを使用したり、または極小のステートマシンを持つ単純なレジスタのチェーンを使用したものなどです。
小規模なバッファ(最大数十エレメント)の場合、個々のレジスタをバッファのチェーンに接続して構成されたFIFOは、リソース要件が小さい単純なインプリメンテーションとなります。
...
信号\code{full}はライタ側で\myref{https://en.wikipedia.org/wiki/Flow_control_(data)}{フロー制御}を行います。
\fi
...
\code{エンプティ}信号は、リーダ側のフロー制御を担当する。
\fi
...
\ifshowtranssecond %(仮訳 12/22 aki)
リスト~\ref{lst:fifo:stage}は1つのバッファを示しています。
...
FSMには2つの状態しかありません。
バッファが\code{空}か\code{満杯}かのどちらかです。
バッファが\code{空}の場合、書き込みは入力データを登録して\code{満杯}状態に変更します。
バッファが\code{満杯}の場合、読み出しはデータを消費して\code{空}の状態にかわります。
IOポートの\code{満杯}と\code{空}は、ライタとリーダのバッファの状態を表しています。
\fi
...
\ifshowtranssecond %(仮訳 12/22 aki)
リスト~\ref{lst:fifo}は、完全なFIFOを示しています。
完全なFIFOは、それぞれのFIFOバッファと同じIOインターフェースを持っています。
\code{BubbleFifo}は、データワードの\code{サイズ}とバッファステージの\code{深さ}をパラメータとして持っています。
\code{深さ}\code{FifoRegister}から\code{深さ}ステージのバブルFIFOを構築することができます。
...
(2) データは完全なFIFO を通ってバブルする必要があるため、入力から出力までのレイテンシは少なくともバッファの数になります。
節~\ref{sec:more:fifo}でFIFOの他の可能な実装を提示紹介します。
\fi
...
\section{A Serial Port}
\fi
\ifshowtranssecond %(仮訳 12/23 aki)
\section{シリアルポート (L10368 仮訳 12/23 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/23 aki)
シリアルポート(\myref{https://en.wikipedia.org/wiki/Universal_asynchronous_receiver-transmitter}{UART}または\myref{https://en.wikipedia.org/wiki/RS-232}{RS-232}とも呼ばれる)は、ラップトップとFPGAボードの間で通信するための最も簡単なオプションの1つです。
その名の通り、データはシリアルに送信されます。
8ビットバイトは次のように送信されます :
1つのスタートビット(0)、8ビットのデータ、最下位ビットが最初に送信され、その後1つまたは2つのストップビット(1)が送信されます。
データが送信されていないときは、出力は１となります。 
図~\ref{fig:uart:wave}に１バイト送信した場合のタイミング図を示します。
\fi
...
\ifshowtranssecond %(仮訳 12/23 aki)
リスト~\ref{lst:uart:tx}はベアボーンのシリアル送信機(\code{Tx})を示しています。
IOポートはシリアルデータが送信される\code{txd}ポートと、送信機がシリアライズして送信する文字を受信できる\code{チャネル}です。
...
\ifshowtranssecond %(仮訳 12/23 aki)
3つのレジスタを使用しています。
(1) データをシフト(シリアライズ)するためのレジスタ(\code{shiftReg})です。
(2) 正しいボーレートを生成するためのカウンタ(\code{cntReg})と
(3) まだシフトアウトする必要があるビット数のカウンタ。
FSMの追加のステートレジスタは必要なく、すべてのステートはこれら3つのレジスタにエンコードされます。
\fi
...
したがって、チャンネルは、\code{cntReg}が0のときだけ準備ができており、シフトアウトするビットが残っていません。
\fi
...
シフトアウトする必要がない場合は、チャネルにデータが含まれているかどうかをチェックします(\code{valid}なポートでシグナルされます)。
そうであれば、シフトアウトされるビット列は、1つのスタートビット(0)、8ビットのデータ、2つのストップビット(1)で構成されます。したがって、ビット数は11に設定されます。
\fi
...
\ifshowtranssecond %(仮訳 12/23 aki)
この非常に小さな送信機は、追加のバッファを持たず、シフトレジスタが空の時と、\code{cntReg}が0の時のクロックサイクルでのみ、新しい文字を受け付けることができます。
...
\ifshowtranssecond %(仮訳 12/23 aki)
\longlist{code/uart_buffer.txt}{A single-byte buffer with a ready/valid interface.}{lst:uart:buffer}
リスト~\ref{lst:uart:buffer}は、バブルFIFO用のFIFOレジスタに似たシングルバイトバッファを示しています。
入力ポートは\code{Channel}インターフェースであり、出力は方向が反転した\code{Channel}インターフェースです。
バッファには、\code{空}か\code{フル}かを示すためのミニマムステートマシンが含まれています。
...
\ifshowtranssecond %(仮訳 12/23 aki)
ステートが\code{空}で、入力データが\code{有効}な場合は、データを登録して\code{フル}ステートに移行します。
ステートが\code{フル}になり、下流側の受信機が\code{ready}になったら、下流側のデータ転送が行われ、状態を\code{空}に戻します。
\fi
...
リスト~\ref{lst:uart:buffered:tx}は、単一のバッファが前にある送信機\code{Tx}の連携示しています。
このバッファは、\code{Tx}がシングルクロックサイクルのみ\code{準備}ができていたという問題を緩和します。
...
\ifshowtranssecond %(仮訳 12/23 aki)
\longlist{code/uart_rx.txt}{A receiver for a serial port.}{lst:uart:rx}
リスト~\ref{lst:uart:rx}にレシーバー (Rx)のコードを示します。
レシーバーはシリアルデータのタイミングを再構築する必要があるので、少し厄介です。
...
8ビットシフトインした後、\code{valReg}に利用可能なバイトを通知します。
\fi
...
\ifshowtranssecond %(仮訳 12/23 aki)
\longlist{code/uart_sender.txt}{Sending ``Hello World!" via the serial port.}{lst:uart:sender}
リスト~\ref{lst:uart:sender}は、フレンドリーなメッセージを送信するシリアルポート送信機の使い方を示しています。
...
各\code{ready}信号で、完全な文字列が送信されるまでカウンタを増加させます。
...
\ifshowtranssecond %(仮訳 12/23 aki)
\longlist{code/uart_echo.txt}{Echoing data on the serial port.}{lst:uart:echo}
リスト~\ref{lst:uart:echo}は、受信機と送信機を接続した場合の使い方を示しています。
...
\fi
\ifshowtranssecond %(仮訳 12/23 aki)
\section{ FIFO設計のバリエーション (L11322 仮訳 12/23 aki)}
\fi
...
\subsection{Parameterizing FIFOs}
\fi
\ifshowtranssecond %(仮訳 12/24 aki)
\subsection{FIFOのパラメータ化 (L1135 仮訳 12/24 aki)}
\fi
...
抽象クラスでは、パラメータの\code{深さ}が有用な値であることもテストしています。
\fi
...
このとき書き込み側で\code{valid}信号に通知します。
...
\ifshowtranssecond %(仮訳 12/24 aki)
抽象的な基底クラスとインターフェースを使用することで、異なるパラメータ(速度、面積、電力、または単純さ)に対して最適化されたFIFOインプリメンテーションとすることができます。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/24 aki)
\subsection{バブルFIFOの再設計 (L11471 仮訳 12/24 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/24 aki)
標準的なready-validインターフェースを使用して、Chiselのデータ型でパラメトリック化することで、節~\ref{sec:fifo}のバブルFIFOを再定義することができます。
\fi
...
\ifshowtranssecond %(仮訳 12/24 aki)
リスト~\ref{lst:fifo:bubble}は、Bubble FIFOを、ready-validインターフェイスでリファクタリングしたものです。
\code{バブルFifo}のプライベートクラスとして\code{Buffer}コンポーネントを内包していることに注意してください。
...
\ifshowtranssecond %(仮訳 12/24 aki)
プロデューサが\code{valid}で、コンシューマが\code{ready}なときに新しいワードを受け付けることができるように、バッファ内の両方のインタフェースを見るようにすることもできる。
...
\fi
\ifshowtranssecond %(仮訳 12/24 aki)
\subsection{ダブルバッファFIFO (L11554 仮訳 12/24 aki)}
\fi
...
バッファが一杯になると、新しいデータがシャドウレジスタに格納され、\code{ready}がデ・アサートされます。
...
\ifshowtranssecond %(仮訳 12/24 aki)
リスト~\ref{lst:fifo:double:buffer}はダブルバッファを示しています。
各バッファ要素が2つのエントリを格納できるので、バッファ要素の半分(\code{depth/2})しか必要ありません。
...
コンシューマが\code{ready}をデ・アサートした場合のみ、キューは一杯になり、バッファは\code{two}の状態になります。
しかし、シングルバブルFIFOと比較すると、キューの再起動に要する時間は、同じバッファー容量の場合の半分のクロック サイクル数しかかかりません。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/24 aki)
\subsection{レジスタメモリ付きFIFO (L11646 仮訳 12/24 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/24 aki)
ソフトウェアエンジニアリングの経験があるかたは、私たちが多くの小さな個々の小さなバッファ要素からハードウェアキューを構築し、すべての要素が並列に実行され、上流および下流の要素とのハンドシェイクを行っていることを不思議に思われるかもしれません。
...
この設定では、固定サイズのFIFOキューは通常\myref{https://en.wikipedia.org/wiki/Circular_buffer}{循環バッファ}として実装されます。
...
リストListing~\ref{lst:fifo:reg:mem}は、メモリと読み書きポインタを使って実装されたFIFOキューを示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/24 aki)
動作したときにインクリメントされて、バッファのラップアラウンドを行うために、ラップカウンターを実装する関数\code{counter}を定義した、同じようにふるまいをする2つのポインターがあります。
カウンタのビット長は、\code{log2Ceil(depth).W}で計算します。
次の値は1のときはインクリメントするか0でラップアラウンドです。
...
\ifshowtranssecond %(仮訳 12/24 aki)
さらに、可能な次の値(インクリメントまたはラップアラウンドの0)も必要なので、この値を\code{counter}関数からも返します。
...
\ifshowtranssecond %(仮訳 12/24 aki)
\noindent 割り当ての左側にある括弧表記を使用して、このようなタプルを分解できます :
\fi
...
\ifshowtranssecond %(仮訳 12/24 aki)
メモリにはChiselデータ型\code{gen}の(\code{Reg(Vec(depth, gen))}を使ったレジスタを使用します。
...
(1) バッファへの書き込み、(2) \code{emptyReg}のデ・アサートを確実に行います。
...
(1) \code{fullReg}がデ・アサートされていることを確認し、(2) 読み出しポインタが次のクロックサイクルで書き込みポインタに追いついた場合はバッファを空にし、(3) 読み出しカウンタをインクリメントするための信号を送ります。
\fi
...
\subsection{FIFO with On-Chip Memory}
\fi
\ifshowtranssecond %(仮訳 12/27 aki)
\subsection{オンチップメモリ付きFIFO (L11872 仮訳 12/27 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 aki)
最後のバージョンのFIFOでは、メモリを表現するためにレジスタファイルを使用していましたが、これは小さなFIFOの場合には良い解決策です。
より大きなFIFOの場合は、オンチップメモリを使用する方が良いでしょう。
リスト~\ref{lst:fifo:mem}は、ストレージに同期メモリを使用したFIFOを示しています。
\fi
...
しかし、レジスタファイルの読み出しが同じクロックサイクルで利用可能であったが、同期オンチップメモリでは、次のクロックサイクルで読み出しの結果を配信される。
\fi
...
最後のデザインでは、FIFOの出力はメモリの読み取りから直接来る場合があります。
このデータパスがデザインのクリティカルパスにある場合、2つのFIFOを組み合わせることで、デザインを簡単にパイプライン化することができます。
リスト~\ref{lst:fifo:comb}はそのような組み合わせを示しています。メモリベースのFIFOの出力には、メモリ読み取りパスを出力から切り離すために、シングルステージのダブルバッファFIFOを追加します。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/27 aki)
\section{演習 (L11980 仮訳 12/27 aki)}
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/27 aki)
\subsection{バブルFIFOを探る (12004 仮訳 12/27 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 aki)
また、FIFOソースには、さまざまな読み書き動作を誘発し、\myref{https://en.wikipedia.org/wiki/Value_change_dump}{value change dump (VCD)}形式の波形を生成するテスタも含まれています。
...
\ifshowtranssecond %(仮訳 12/27 aki)
最初のサイクルでは、テスターは1つの単語を書きます。そのワードが\emph{bubble FIFO}と名付けられたFIFOを通って伝搬する様子を波形で観察できます。
...
\ifshowtranssecond %(仮訳 12/27 aki)
テストの最後には、最大速度で書き込みと読み込みを試みるループが含まれています。
...
\ifshowtranssecond %(仮訳 12/27 aki)
これらの欠点は、\myref{https://en.wikipedia.org/wiki/Circular_buffer}{循環バッファ}を用いたFIFO実装によって解決することができる。
循環バッファは、メモリと読み書きポインタを用いて実装することができる。
...
\fi
\ifshowtranssecond %(仮訳 12/27 aki)
\subsection{UART (L12162 仮訳 12/27 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/27 aki)
USB UARTでは、大抵の場合、これは\code{/dev/ttyUSB0}でしょう。
...
\ifshowtranssecond %(仮訳 12/27 aki)
LEDの点滅の例をUARTで拡張して、LEDが消灯している時と点灯している時にシリアル・ラインに0と1を書き込みます。
...
\ifshowtranssecond %(仮訳 12/27 aki)
サンプルコードには、Tx用のバッファが1つしか含まれていません。
送信機と受信機にバッファリングを追加するために実装したFIFOを自由に追加してください。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/27 aki)
\subsection{FIFO探査 (L12296 仮訳 12/27 aki)}
\fi
...
この単純化により、リスト~\ref{lst:fifo:reg:mem}の例にあるカウンター関数と、同じポインタ値を持つ空や満杯の扱いを避けることができます。
...
\chapter{Design of a Processor}
\fi
\ifshowtranssecond %(仮訳 12/28 aki)
\chapter{プロセッサの設計 (L12361 仮訳 12/27 aki)} %(仮訳 12/28 aki  12章 ========================================================================)
\fi
...
マイクロプロセッサの設計、シミュレーション、テストである。
このプロジェクトを管理しやすくするために、単純なアキュムレータマシンを設計します。
このプロセッサは\myref{https://leros-dev.github.io/}{Leros}~\cite{leros:arcs2019}と呼ばれ、オープンソースで\url{https://github.com/leros-dev/leros}から入手可能です。
...
命令の記述は1ページに収まるようになっており、表~\ref{tab:leros:isa}を参照してください。
その中で\code{A}はアキュムレータ、\code{PC}はプログラムカウンタ、\code{i}は即時値(0〜255)、\code{Rn}はレジスタ\code{n}(0〜255)、\code{o}は\code{PC}との相対的な分岐オフセット、\code{AR}はメモリアクセス用のアドレスレジスタを表しています。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/28 aki)
\section{ALUから始める (L12464 仮訳 12/28 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
プロセッサの中心的な構成要素は、\myref{https://en.wikipedia.org/wiki/Arithmetic_logic_unit}{arithmetic logic unit}、略してALUです。
そこで、まずALUのコーディングとテストベンチから始めます。
まず、ALUのさまざまな演算を表す\code{Enum}を定義します : 
\fi
...
リスト~\ref{lst:leros-alu}はALUを示しています。
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
テストのため、リスト~\ref{lst:leros-alu-scala}に示すように、ALU関数を平易なScalaで記述したものを示します。
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent Chiselで記述されたハードウェアの重複は、Scalaの実装仕様によりエラー検出をしません。少なくとも何らかの確かな確認にはなります。
テストベクタとしていくつかのごくまれな値を使用します : 
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent 32ビット引数に対するすべてに対する網羅的なテストは不可能であり、これが入力値としていくつかのまれな境界値を選択した理由です。
まれな境界値に対するテストの他に、ランダムな入力に対するテストも有用です : 
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent Lerosプロジェクトにテストを実行するには、次のようにします。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/28 aki)
\section{命令のデコード (L12644 仮訳 12/28 aki)}
\fi
...
まず最初に、独自のScalaクラスと\emph{共有}パッケージを使って命令エンコーディングを定義します。
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent デコードコンポーネントに出力の\code{バンドル}を定義します。バンドルは後で部分的にALUに供給されます。
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent デコードは8ビットのオペコードを入力として受け取り、デコードされた信号を出力として出力します。
...
\ifshowtranssecond %(仮訳 12/28 aki)
\noindent デコードは、オペコードに含まれる命令の部分に対する大きなスイッチ文に過ぎません(Lerosでは、ほとんどの命令は上位8ビットです)。
\fi
...
\fi
\ifshowtranssecond %(仮訳 12/28 aki)
\section{アセンブラ命令 (L12729 仮訳 12/28 aki)}
\fi
...
しかし、最初のテストでは、いくつかの命令をハードコーディングしてScalaの配列に入れ、命令メモリの初期化に使用します。
\fi
...
(1) 整数定数(10進数または16進数表記が可能)と、(2) レジスタ番号を読み込みます。
\fi
...
\ifshowtranssecond %(仮訳 12/28 aki)
リスト~\ref{lst:leros-asm-match}にLerosアセンブラのコア部分を示します。
...
\fi
\ifshowtranssecond %(仮訳 12/28 aki)
\section{演習 (L12833 仮訳 12/28 aki)}
\fi
...
\ifshowtransfirst %(自動翻訳)
1つのオプションは、章を再読し、内のすべてのソースコードと一緒に読んで、テストケースを実行し、それを破壊することにより、コードとフィドルとテストが失敗することを確認することです。
\fi

\ifshowtranssecond %(仮訳 12/28 aki)
1つの選択肢は、この章を読み直して、\myref{https://github.com/leros-dev/leros}{Leros リポジトリ}にあるすべてのソースコードと一緒に読み、テストが失敗するほどコードをいじくり回し、コードを壊してテストケースを実行し、失敗することを確認することです。
\fi
...
Lerosプロセッサと必要なツールを構築する方法のデモンストレーションにより、プロセッサの設計と実装は魔法の芸術ではなく、非常に楽しいエンジニアリングであることがわかっていただけるでしょう
\fi
