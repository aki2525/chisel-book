...
\fi
\ifshowtranssecond %(校正)
\chapter{コミュニケートステートマシン (L8903 TODO)} %(仮訳 12/20 校正1 01/01 aki  第9章  ========================================================================)
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{ライトフラッシャーの例 (L8944 仮訳 12/20 校正1 01/01 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/20 校正1 01/01 aki)
\begin{itemize}
\item \code{start}が1クロックサイクルの間ハイの時、点滅シーケンスを開始します。
\item シーケンスは3回点滅する。
\item 点滅の間、\code{light}は、6クロックサイクル\emph{on}に、4クロックサイクル\emph{off}になります。
\item シーケンスの後、FSMは\code{light}を\emph{off}にして次のスタートを待ちます。
\end{itemize}
\fi
...
入力待ちの初期状態が1つ、3種類の\emph{on}状態が$3 \times 6$と、\emph{off}状態が$2 \times 4$あります。
...
\ifshowtranssecond %(仮訳 校正1 01/01 aki)
\begin{itemize}
...
\fi
\ifshowtranssecond %(校正)
\section{データパスを持つステートマシン (L9194 仮訳 12/20 校正1 01/01 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{ポップカウントの例 (L9242 仮訳 12/20 校正1 01/01 aki)}
\fi
...
ダウンカウンタがゼロになると計算が終了し、FSMは\code{終了}の状態に移行します。
...
\ifshowtranssecond %(仮訳 12/20 校正1 01/01 aki)
\code{ロード}信号では、\code{regData}レジスタに入力が読み込まれ、\code{regPopCount}レジスタが0にリセットされ、カウンタレジスタ\code{regCount}が実行するシフト数に設定されます。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{Ready-Valid インターフェース (L9462 仮訳 12/21 校正1 01/01 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/21 校正1 01/01 aki)
...
送信側は\code{データ}があるときに\code{バリッド}をアサートし、受信側は1ワードのデータを受信する準備ができているときに\code{レディ}をアサートします。
...
\ifshowtranssecond %(仮訳 12/21 校正1 01/01 aki)
図~\ref{fig:ready_valid1}は、送信機がデータを持つ前に(クロックサイクル１から)受信機が\code{レディ}信号を出すready-validトランザクションのタイミング図を示しています。
...
\ifshowtranssecond %(仮訳 12/21 校正1 01/01 aki)
図~\ref{fig:ready_valid2}は、ready-validインタフェースの更なるバリエーションを示しています。
...
\ifshowtranssecond %(仮訳 12/21 校正1 01/01 aki)
1つの疑問が残るのは、アクティブな状態でデータ転送が行われて\emph{いない}場合に、\code{レディ}または\code{バリッド}な状態がデアサートされる可能性があるかどうかです。
