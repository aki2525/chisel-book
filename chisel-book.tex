...
\ifshowtranssecond %(仮訳 12/06 aki)
順序回路は、出力が入力\emph{と}前の値に依存する回路です。
ここでは同期設計（クロックドデザイン）を前提として、順序回路とは同期式順序回路を意味します。
\footnote{非同期ロジックやフィードバックで順序回路を構築することができますが、これは特定のニッチな話題で、Chiselで表現することはできません}
順序回路を構築するためには、状態を格納できる要素が必要で、レジスタと呼ばれます。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{レジスター (L5743 仮訳 12/06 aki)}
\fi
...
我々は、次の回路図でクロック信号を省略します。(訳注:次ってどれ?本書の以降??)
...
リセット値を、たとえばゼロとしてレジスタコンストラクタ のReginitにパラメータとして与えます。
...
同期リセットの場合は、D フリップフロップの変更は必要なく、入力にマルチプレクサ\footnote{現在のFPGA フリップフロップには同期リセット入力がありますから、マルチプレクサに追加のリソースは必要ありません。)を追加してリセット時の初期化値とデータ値を選択するだけです。
\fi
...
\ifshowtranssecond %(仮訳 12/06 aki)
図〜\ref{fig:register-reset}は、リセットがマルチプレクサを駆動する同期リセット付きレジスタの回路図を示しています。
...
そのふるまいは、時間の経過とともに変化する信号を示すダイアグラムによってみることができる。このようなダイアグラムは、ウェーブフォームまたは\myref{https://en.wikipedia.org/wiki/Digital_timing_diagram}{timing diagram}と呼ばれています。
\fi
...
\ifshowtranssecond %(仮訳 12/06 aki)
図〜\ref{fig:register-wave}に、リセットと一部の入力データが適用されたレジスタのウェーブフォームを示す。
時間は左から右へと進む。
...
ほとんどの場合、イネーブルをハイ(\code{true})にすると、レジスタは1クロックサイクルの遅延で入力に従います。
...
\fi
\ifshowtranssecond %(校正)
\section{カウンター (L6109 仮訳 12/06 aki})
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{カウントアップとダウン (L6268 仮訳 12/06 aki})
\fi
...
\ifshowtranssecond %(仮訳 12/07 aki)
...
\fi
\ifshowtranssecond %(校正)
\subsection{「オタク」カウンター (L6538 仮訳 12/07 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{タイマー (L6610 仮訳 12/08 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 aki)
図〜\ref{fig:timer}に、タイマのブロック図を示す。
...
\ifshowtranssecond %(仮訳 12/08 aki)
もう少し簡潔なコードにするなら、中間ワイヤの\code{next}を使用せずに、マルチプレクサの値を直接レジスタ\code{cntReg}に代入することができます。
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{パルス幅変調(PWM) (L6721 仮訳 12/08 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 aki)
\noindent 再利用可能で軽量なコンポーネントを提供するために、PWMジェネレータに関数を使用しました。
...
実数の除算はハードウェア上では非常に高価なので、単に右に10ビットシフトするだけで、$2^{10} = 1024$による除算と同じになります。
...
\fi
\ifshowtranssecond %(校正)
\section{シフトレジスタ (L6919 仮訳 12/08 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/08 aki)
シフトレジスタは、シリアルデータからパラレルデータへの変換やパラレルデータからシリアルデータへの変換に使用されます。
...
\fi
\ifshowtranssecond %(校正)
\subsection{パラレル出力付きシフトレジスタ (L6992 仮訳 12/08 aki)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{パラレルロード付きシフトレジスタ (L7067 仮訳 12/08 aki}}
\fi
...
\ifshowtranssecond %(仮訳 12/09 aki)
...
\fi
\ifshowtranssecond %(校正)
\section{メモリー (L7128 仮訳 12/09 aki)}
\fi
...
\ifshowtranssecond %(仮訳 12/09 aki)
図〜\ref{fig:memory}に、同期メモリの回路図を示す。
...
\ifshowtranssecond %(仮訳 12/09 aki)
新しく書き込んだ値を読み出したい場合は、アドレスが等しいことを検出して書き込みデータを送る\emph{フォワード(転送)}回路を構築すればできる。
...
2つの入力アドレス((\code{wrAddr}と\code{rdAddr}) を比較し、フォワード(転送)条件で\code{wrEna}が真であるかどうかを確認します。
その条件も1クロックサイクル遅れます。
...
このカウンタをFPGAボードのクロックに直接接続すると、16個の数字がすべて重なってみえます(7セグメントすべてが点灯します)。
...
\ifshowtranssecond %(仮訳 12/09 aki)
ジェネレータ関数でスレッショルドと波形(三角関数またはサイン関数)を設定しPWM波形をつくってみましょう。
三角関数は上下にカウントすることで作成できます。
サイン関数は、数行のScalaコードでルックアップテーブルを生成し使えばよいでしょう(see Section~\ref{sec:gen:comb:logic})。
