...
そのふるまいは、時間の経過とともに変化する信号を示すダイアグラムによってみることができます。このようなダイアグラムは、波形または\myref{https://en.wikipedia.org/wiki/Digital_timing_diagram}{timing diagram}と呼ばれています。
\fi
...
マルチプレクサへの入力の1つは、レジスタの出力からのフィードバックです。
\fi
...
\fi
\ifshowtranssecond %(校正)
\chapter{入力処理 (L7519 12/30 初回校正済み)}%(校正  第7章  ========================================================================)
\fi
...
\ifshowtranssecond %(校正 12/8 diningyo)
...
\fi
\ifshowtranssecond %(校正)
\section{非同期入力 (L7502 初回校正済 12/8 mune10, aki, diningyo)}
\fi

\index{Asynchronous Input}
\index{Metastability}

\ifshowtranssecond %(追記 12/30 mune10)
\index{非同期入力}
\index{メタステーブル}
\fi
...
\ifshowtranssecond %(校正 1/1 mune10)
\begin{figure}
  \centering
  \includegraphics[scale=\scale]{figures/synchronizer}
...
クロックに同期させるために、リセット信号のディアサートが定まることが必要です。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{デバウンス (L7640 初回校正済 12/8 mune10, aki, diningyo)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{ 入力信号のフィルタリング (L7872 12/30 mune10)}
\fi
...
\ifshowtranssecond %(校正 12/30 mune10)
...
\fi
\ifshowtranssecond %(校正)
\section{入力処理と関数の組み合わせ (L7936 12/30 mune10)}
\fi
...
セクション\ref{sec:functions}では、完全なモジュールではなく、軽量なChisel関数の中に小さなビルディングブロックを抽象化する方法を示しました。
...
\fi
\ifshowtranssecond %(校正)
\chapter{ハードウェアジェネレータ (L9245 初回校正済 12/22 mune10, diningyo)} %(校正  第10章  ========================================================================)
\fi
...
\ifshowtranssecond %(校正 12/22 diningyo)
Chiselの強みはいわゆるハードウェア・ジェネレータを書けることです。
...
これにより、ハードウェア・ジェネレータを同じ言語で記述し、それをChiselの回路生成の一部として実行できます。
\fi
...
\fi
\ifshowtranssecond %(校正)
\section{パラメータを使って設定する (L8932 12/22- 校正済 mune10, diningyo)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{シンプルなパラメータ (L9816 12/22 初回校正済み diningyo)}
\fi
...
\fi
\ifshowtranssecond %(校正)
\subsection{型パラメータを持つ関数 (L9865 12/22-12/23 初回校正済み diningyo)}
\fi
...
\ifshowtranssecond %(校正 12/22 diningyo)
設定パラメータとしてビット幅を持つことは、ハードウェア・ジェネレータとしての出発点となります。
...
\ifshowtranssecond %(校正 12/22 diningyo)
型パラメータとしてChiseの型を使ってパラメタライズすることが可能です。角括弧\code{[T <: Data]}を使った表現で、\code{Data}とそのサブクラスを型パラメータ\code{T}として定義します。
...
\ifshowtranssecond %(校正 12/23 diningyo)
ここで定義したのマルチプレクサ関数（\code{myMux}）は次の3つのパラメータを持ちます。１つ目はブール値の条件、２つ目は条件が真の場合のパスで、３つ目は条件が偽の場合のパスです。
...
\fi
\ifshowtranssecond %(校正)
\subsection{型パラメータを持つモジュール (L10036 12/23 校正済 diningyo)}
\fi
...
関数で型パラメータを使ったときと同様に、モジュールのコンストラクタでも型パラメータ\code{T}を追加します。さらにこの型をパラメータの１つとするコンストラクタが必要になります。加えてこの例では、ルータのポート数を可変にするための変数も追加してあります。
\fi
...
\ifshowtranssecond %(校正 12/29 diningyo)
...
\fi
\ifshowtranssecond %(校正)
\section{組合せ論理回路の生成 (L10203 12/29 dinnigyo)}
\fi
...
\ifshowtranssecond %(校正 12/31 mune10)
...
\fi
\ifshowtranssecond %(校正)
\section{継承を利用する (L10372 12/29 diningyo)}
\fi
...
\ifshowtranssecond %(校正 12/30 diningyo)
...
\code{TickerTester}は複数のパラメータを持ちます。1つ目の型パラメータ\code{[T <: Ticker]}は\code{Ticker}は又は\code{T}または\code{Ticker}を継承する任意のクラスを受け取れまます。2つ目は\code{T}の型を持つDUTで、3つ目は期待するカウントパルスのクロック周期です。
...
\fi
\ifshowtranssecond %(校正)
\section{関数型プログラミングによるハードウェア生成 (L19523 12/30 mune10)}
\fi
...
論理合成ツールの加算器チェーンを再配置(最適化)機能を信用しないのであれば、Chiselの\code{reduceTree}メソッドを使ってツリー型の加算器を生成することができます。
\fi
