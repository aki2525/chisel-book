...
入力と出力には、ready-valid ハンドシェイクを使用します。
...
\ifshowtranssecond %(仮訳 12/21 aki)
レディバリッドインタフェース~\cite[p.~480]{dally:vhdl:2016}は、送信側(プロデューサ)では\code{データ}と\code{バリッド}信号、受信側(コンシューマ)では\code{レディ}信号で構成される単純なフロー制御インタフェースです。
図~\ref{fig:readyvalid}は、レディバリッド接続を示しています。
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid1}は、受信機が送信機がデータを持つ前に(クロックサイクル１から)受信機が\code{レディ}信号を出すレディバリッドトランザクションのタイミング図を示しています。
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid2}は、送信者が受信機の準備が整う前に(クロックサイクル１から)\code{バリッド}な信号を送信するレディバリッドトランザクションのタイミング図を示しています。
...
\ifshowtranssecond %(仮訳 12/21 aki)
図~\ref{fig:ready_valid2}は、レディバリッドインタフェースの更なるバリエーションを示している。
...
この動作が許されるかどうかは、レディバリッドインタフェースの一部ではなく、インタフェースの具体的な使用法によって定義される必要があります。
\fi
...
\ifshowtranssecond %(仮訳 12/21 aki)
AXIは次のバスの各部分に1つのレディバリッドインターフェースを使用します : 
...
FSMの追加のステートレジスタは必要なく、すべてのステートはこれら3つのレジスタにエンコードされます。\fi
